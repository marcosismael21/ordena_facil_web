  ...
  <v-container>
    ...
    <v-dialog v-model="isConfirmDialogVisible" persistent width="500">
      <v-card title="Confirmar eliminación">
        <v-card-text>
          ¿Está seguro que desea eliminar este tipo de platillo? Esta acción no se puede deshacer.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
const getData = async () => {
  try {
    const response = await $fetch(runtimeConfig.public.apiBase + "/tipoPlatillo", {
      method: "GET",
      headers: {
...
const handleCreate = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      snackbarMessage.value = "Tipo de platillo creado exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al crear el tipo de platillo"
    isSnackbarVisible.value = true
  }
}
...
const handleUpdate = async () => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoPlatillo/${editingItemId.value}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      snackbarColor.value = "success"
      snackbarMessage.value = "Tipo de platillo actualizado exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al actualizar el tipo de platillo"
    isSnackbarVisible.value = true
  }
}
...
const handleDelete = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoPlatillo/${deleteItemId.value}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      ...
      snackbarColor.value = "success"
      snackbarMessage.value = "Tipo de platillo eliminado exitosamente"
      isSnackbarVisible.value = true
...
const editItem = async (id) => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoPlatillo/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al cargar los datos del tipo de platillo"
    isSnackbarVisible.value = true
  }
}
