<template>
  <v-container>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="form">
            <v-row>
              <v-col cols="6">
                <v-text-field v-model="formData.descripcion" label="Nombre de la mesa"
                              placeholder="Ej. Mesa 1, #1" :rules="[rules.required, rules.text]"
                              required></v-text-field>
              </v-col>
              <v-col cols="6">
                <v-text-field v-model="formData.cupos" label="Cantidad de personas"
                              placeholder="Ej. 4" :rules="[rules.required, rules.cantidad]" required></v-text-field>
              </v-col>
              <v-col cols="12">
    ...
    <v-dialog v-model="dialogEditar" width="500">
      <v-card>
        <v-card-title>
          Editar Tipo de Platillo
        </v-card-title>
        <v-card-text>
          <v-form ref="form">
            <v-row>
              <v-col cols="6">
                <v-text-field v-model="formData.descripcion" label="Nombre del Tipo de Platillo"
                              placeholder="Ej. Pizzas" :rules="[rules.required, rules.text]" required></v-text-field>
              </v-col>
              <v-col cols="6">
                <v-text-field v-model="formData.cupos" label="Cantidad de personas"
                              placeholder="Ej. 4" :rules="[rules.required, rules.cantidad]" required></v-text-field>
...
const rules = {
  ...
  cantidad: (v) => {
    ...
    if (!/^(?:[1-9]\d{0,2})$/.test(v)) return "Solo se permiten números enteros de 1 a 3 cifras, sin ceros a la izquierda"
    return true
  },
...
const getData = async () => {
  try {
    const response = await $fetch(runtimeConfig.public.apiBase + "/mesa", {
      method: "GET",
      headers: {
...
const handleCreate = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      snackbarMessage.value = "Mesa creada exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al crear la mesa"
    isSnackbarVisible.value = true
  }
}
...
const handleUpdate = async () => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/mesa/${editingItemId.value}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      snackbarColor.value = "success"
      snackbarMessage.value = " Mesa actualizada exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al actualizar la mesa"
    isSnackbarVisible.value = true
  }
}
...
const handleDelete = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${runtimeConfig.public.apiBase}/mesa/${deleteItemId.value}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      ...
      snackbarColor.value = "success"
      snackbarMessage.value = "Mesa eliminada exitosamente"
      isSnackbarVisible.value = true
...
const editItem = async (id) => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/mesa/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al cargar los datos de la mesa"
    isSnackbarVisible.value = true
  }
}
