<template>
  <v-container>
    ...
    <v-dialog v-model="dialogCrear" width="500">
      <v-card>
        <v-card-title>
          Nuevo Tipo de Medidas
        </v-card-title>
        <v-card-text>
          <v-form ref="form">
            <v-row>
              <v-col cols="12">
                <v-text-field v-model="formData.descripcion" label="Nombre del Tipo de Medidas"
                  placeholder="Ej. Libras" :rules="[rules.required, rules.text]" required></v-text-field>
              </v-col>
              <v-col cols="12">
    ...
    <v-dialog v-model="isConfirmDialogVisible" persistent width="500">
      <v-card title="Confirmar eliminación">
        <v-card-text>
          ¿Está seguro que desea eliminar este tipo de medida? Esta acción no se puede deshacer.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
const getData = async () => {
  try {
    const response = await $fetch(runtimeConfig.public.apiBase + "/tipoMedida", {
      method: "GET",
      headers: {
...
const handleCreate = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      snackbarMessage.value = "Tipo de medida creado exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al crear el tipo de medida"
    isSnackbarVisible.value = true
  }
}
...
const handleUpdate = async () => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoMedida/${editingItemId.value}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      snackbarColor.value = "success"
      snackbarMessage.value = "Tipo de medida actualizado exitosamente"
      isSnackbarVisible.value = true
      closeDialog()
      getData()
    } else {
...
const handleDelete = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoMedida/${deleteItemId.value}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      ...
      snackbarColor.value = "success"
      snackbarMessage.value = "Tipo de medida eliminado exitosamente"
      isSnackbarVisible.value = true
...
const editItem = async (id) => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.apiBase}/tipoMedida/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al cargar los datos del tipo de medida"
    isSnackbarVisible.value = true
  }
}
