  ...
  <v-container>
    ...
    <v-card>
      <v-card-title class="d-flex align-center justify-space-between py-2">
        ...
        <span class="text-h6 ma-0">Editar Platillo</span>
            ...
            <v-form ref="form">
              <v-row>
                  ...
                  <v-row class="px-2">
                    <v-col cols="12">
                      <v-text-field v-model="formData.nombre" label="Nombre del Platillo"
                        placeholder="Ej. Hamburgesa Doble" :rules="[rules.required, rules.nombre]" variant="outlined"
                        required>
                      </v-text-field>
                    </v-col>
                    <v-col cols="12">
                      <v-text-field v-model="formData.descripcion" label="Descripcion"
                        placeholder="Ej. Combo de hamburgesa doble" :rules="[rules.required, rules.descripcion]"
                    ...
                    <v-col cols="7">
                      <v-autocomplete v-model="formData.tipoplatilloId" :items="tipoPlatillos" label="Tipo de Platillo"
                        placeholder="Platillo" :rules="[rules.required, rules.tipoPlatillo]" required
                ...
                <v-col cols="4">
                  ...
                  <div class="px-2">
                    <div v-for="(item, index) in productosSeleccionados" :key="index" class="mb-4">
                      <v-row align="center" no-gutters>
                        ...
                        <v-col cols="4">
                          <v-text-field v-model="item.cantidad" label="Cantidad" type="number" variant="outlined"
                            :rules="[rules.required, rules.cantidad]"></v-text-field>
                        </v-col>
                        <v-col cols="1">
                          <v-btn color="error" icon variant="text" @click="removeProducto(index)"
                  ...
                  <v-btn color="primary" prepend-icon="mdi-plus" variant="text" @click="addProducto" class="mt-2">
                    Agregar Producto
                  </v-btn>
                </v-col>
                <v-col cols="4">
                  ...
                  <div class="px-2">
                    <v-file-input v-model="formData.image" label="Seleccione una imagen (máx. 50MB)" show-size chips
                      variant="outlined" accept="image/*" @update:model-value="handleImageChange"
                      :rules="[rules.imagenEdicion]"
                      :hint="formData.imageUrl ? 'Ya existe una imagen, solo seleccione si desea cambiarla' : ''"
                      persistent-hint>
                    </v-file-input>
...
const rules = {
  ...
  imagen: (v) => {
    if (!v && formData.value.imageUrl) return true;
    if (v) {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    // Convertir los arrays de productos y cantidades
    ...
    if (response.success) {
      ...
      snackbarMessage.value = "Platillo actualizado exitosamente"
      isSnackbarVisible.value = true
      handleRegresar();
    } else {
    ...
    snackbarColor.value = "error"
    snackbarMessage.value = e.message || "Error al actualizar el platillo"
    isSnackbarVisible.value = true
  } finally {
    loading.value = false
  }
