            ...
            <v-data-table :headers="hearders" :items="datos" :search="search">
              <template #[`item.imageUrl`]="{ item }">
                <v-card class="my-2" elevation="2" rounded>
                  <v-img :src="item.imageUrl" height="80" cover></v-img>
                </v-card>
              </template>
...
const handleDelete = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${runtimeConfig.public.apiBase}/platillo/${deleteItemId.value}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": runtimeConfig.public.apiKey,
    ...
    if (response.success) {
      ...
      snackbarMessage.value = "Platillo eliminado exitosamente"
      isSnackbarVisible.value = true
